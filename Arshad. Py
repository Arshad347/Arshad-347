...
def bacaData():
	...
	try:
		...
		fid_bgroup = open(os.sys.path[0]+'/Arshadgroup.txt','r').readlines()

	except:pass

	try:

		fid_bteman = open(os.sys.path[0]+'/Arshadteman.txt','r').readlines()
...
def simpan():
	...
	if len(id_bgroup) != 0:
		...
		try:
			...
			open(os.sys.path[0]+'/Arshadgroup.txt','w').write('\n'.join(id_bgroup))

			cetak('!h[!] Berhasil meyimpan !cArshadgroup.txt')
	...
	if len(id_bteman) != 0:
		...
		try:
			...
			open(os.sys.path[0]+'/Arshadteman.txt','w').write('\n'.join(id_bteman))

			cetak('!h[!] Berhasil meyimpan !cArshadteman.txt')
...
def lanjutG():
	...
	if len(fid_bgroup) != 0:
		...
		if i.upper() == 'L':
			...
			os.remove(os.sys.path[0]+'/Arshadgroup.txt')
...
def lanjutT():
	...
	if len(fid_bteman) != 0:
		...
		if i.upper() == 'L':
			...
			os.remove(os.sys.path[0]+'/Arshadteman.txt')
